...
export default function FollowingPage() {
  ...
  const fetchFollowingPosts = async (
    ...
    try {
      const response = await fetch(
        `/api/posts?following=true&page=${pageNum}&limit=10`,
        {
          headers: {
  ...
  const fetchFollowingUsers = async () => {
    ...
    try {
      const response = await fetch(
        `/api/users/${dbUser?.id}/following?limit=6`,
        {
          headers: {
            "Content-Type": "application/json",
          },
  ...
  const handleRefresh = async () => {
    ...
    setPage(1);
    await Promise.all([fetchFollowingPosts(1, false), fetchFollowingUsers()]);
              ...
              <>
                ...
                {hasMore && (
                  <div className="flex justify-center mt-8">
                    <Button onClick={loadMorePosts}>{t("loadMore")}</Button>
                  </div>
                )}
              </>
            )}
            ...
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  ...
                  {tFollowing("followingSection.title", {
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-muted-foreground">
                      {tFollowing("activitySection.following")}
                    </span>
                    <span className="font-medium">{followingUsers.length}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-muted-foreground">
                      {tFollowing("activitySection.recentPosts")}
                    </span>
                    <span className="font-medium">{posts.length}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-muted-foreground">
                      {tFollowing("activitySection.lastUpdated")}
                    </span>
                    <span className="font-medium text-sm">
                      {tFollowing("activitySection.justNow")}
                    </span>
                  </div>
                </div>
              </CardContent>
