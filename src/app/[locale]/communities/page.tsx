...
import { LazyCommunityCreateDialog, LazyGroupCreateDialog, LazyCommunityFeed, LazyGroupFeed } from '@/components/lazy';
...
import { Search, Users, Plus, BookOpen, Hash, Globe, Lock } from 'lucide-react';
...
export default function CommunitiesPage() {
  ...
  const handleSearchCommunities = async () => {
    ...
    try {
      const response = await fetch(`/api/communities?search=${encodeURIComponent(searchQuery)}&limit=50`);
      if (response.ok) {
        const data = await response.json();
        setCommunities(data.communities || []);
      }
      ...
      {!viewingCommunity && !viewingGroup && (
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h1 className="text-4xl font-bold mb-2">Communities & Groups</h1>
              <p className="text-muted-foreground text-lg">
                Join communities to collaborate with other students or create private groups for close friends
              </p>
            </div>
            {!isGuest && (
              <div className="flex gap-2">
                <LazyCommunityCreateDialog onCreateCommunity={handleCreateCommunity} />
              ...
              <CardContent>
                <div className="space-y-3">
                  ...
                  {!isGuest && (
                    <>
                      <div className="flex justify-between items-center">
                        <span className="text-sm text-muted-foreground">My Communities</span>
                        <Badge variant="secondary">{myCommunities.length}</Badge>
                      </div>
                      <div className="flex justify-between items-center">
                ...
                <CardContent>
                  <div className="space-y-3">
                    {communities
                      ...
                      .slice(0, 3)
                      .map((community) => (
                        <div key={community.id} className="flex items-center gap-3 p-2 rounded-lg hover:bg-muted cursor-pointer">
