...
const InnerThemeBridge: React.FC<{ children: React.ReactNode }> = ({
  children,
}) => {
  const {
    theme: nextTheme,
  ...
  const [customTheme, setCustomTheme] = useState<
    "retro-light" | "retro-dark" | "glass-light" | "glass-dark" | null
  >(null);

  const customThemeClasses = new Set([
    "retro-light",
    "retro-dark",
    "glass-light",
    "glass-dark",
  ...
  const isCustomTheme =
    !!customTheme || customThemeClasses.has(nextTheme as string);
  const currentTheme = isCustomTheme
    ? customTheme ||
      (nextTheme as "retro-light" | "retro-dark" | "glass-light" | "glass-dark")
    : nextTheme;
  const resolvedTheme = isCustomTheme
    ? customTheme ||
      (nextTheme as "retro-light" | "retro-dark" | "glass-light" | "glass-dark")
    : nextResolvedTheme;
  ...
  useEffect(() => {
    ...
    if (stored && customThemeClasses.has(stored)) {
      setCustomTheme(
        stored as "retro-light" | "retro-dark" | "glass-light" | "glass-dark",
      );
  ...
  useEffect(() => {
    if (isCustomTheme) {
      const cls = (customTheme || (nextTheme as string)) as
        | "retro-light"
        | "retro-dark"
        | "glass-light"
        | "glass-dark";
  ...
  const setTheme = (
    ...
    if (
      ...
      (newTheme.startsWith("retro-") || newTheme.startsWith("glass-"))
    ) {
      setCustomTheme(
        newTheme as "retro-light" | "retro-dark" | "glass-light" | "glass-dark",
      );
