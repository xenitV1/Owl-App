          ...
          <CardContent className="p-4">
            <div className="space-y-3">
              ...
              {q.options && q.options.length > 0 && (
                <div className="space-y-2">
                  <p className="text-sm text-muted-foreground font-medium">{t('options')}:</p>
                    ...
                    {q.options.map((option, optIndex) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <p className={`text-sm flex-1 ${isCorrect ? 'font-medium text-green-700 dark:text-green-400' : ''}`}>
                            {option}
                          </p>
                        </div>
                      );
  ...
  return (
    <div className="space-y-4">
      <Card className="bg-primary/5 border-primary/20">
        <CardHeader>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <Badge variant="secondary">{t(`contentTypes.${content.type}`)}</Badge>
            <Badge variant="outline">{t(`ageGroups.${content.metadata.ageGroup}`)}</Badge>
      ...
      <ScrollArea className="h-[400px] rounded-md border p-4">
        {content.type === 'flashcards' && renderFlashcards(content.content as Flashcard[])}
        {content.type === 'questions' && renderQuestions(content.content as Question[])}
